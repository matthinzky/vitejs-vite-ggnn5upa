...
export default function LeagueDashboard({ league, user, onBack, onUpdate }) {
  ...
  const tabs = [
    ...
    ...(isAdmin ? [{ id: 'admin', label: 'Pannello admin', icon: '⚙️' }] : []),
  ];
  ...
  return (
    <div style={{ maxWidth: 700, margin: '0 auto' }}>
      ...
      >
        {tabs.map((item) => (
          <button
            ...
            style={{
              background: view === item.id ? '#1e3a5f' : '#1a2535',
              border: `1px solid ${view === item.id ? '#2563eb' : '#2a3a50'}`,
              borderRadius: 12,
              padding: '1rem',
              color: '#e8f0fe',
              cursor: 'pointer',
      ...
      {view === 'home' && (
        ...
        >
          <h3 style={{ color: '#e8f0fe', marginTop: 0 }}>
            Benvenuto in {league.name}
          </h3>
          <div
            style={{
              display: 'grid',
          ...
          >
            ...
            >
              <div style={{ color: '#7a8fa6', fontSize: 13 }}>
                La tua squadra
              </div>
              <div
                style={{
      ...
      {view === 'members' && (
        ...
        >
          ...
          {league.members?.map((m) => (
            ...
            >
              <div>
                <div style={{ color: '#e8f0fe', fontWeight: 500 }}>
                  {m.name || m.user_id}
                </div>
                <div style={{ color: '#7a8fa6', fontSize: 13 }}>
                  {m.team_name || 'Squadra non impostata'}
                </div>
              </div>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                ...
                {isAdmin && !m.is_admin && (
                  <button
                    onClick={() => promoteAdmin(m.user_id)}
                    style={{
                      background: 'transparent',
                      border: '1px solid #2a3a50',
                      borderRadius: 6,
